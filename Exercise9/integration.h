...
//Genera un numero casuale distribuito secondo una gaussiana usando il teorema limite centrale
long double gauss(long double a, long double b, int N) //N = numero su cui fare media
{
	long double result = 0.;
	for (int i = 0; i < N; ++i)
	{
		result+=uniformRandom(a,b);
	}
	return result / N;
}